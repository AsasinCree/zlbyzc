		...
		//	System.out.print(" v="+Vprevlength);
			while (!is_done) {
				...
				if (variation < thresh) {
					...
					this.mf.output.append("经马尔科夫决策得到不同状态转移的最优行动方案：" + "\r\n");
...
//				policy[i]=policy[i]+1;//由于数组的下标第一个为0，所以要加1
				...
				this.mf.output.append(this.mf.panelAA.defaultModelFirst.getValueAt(i+1, 0)+":     "+this.mf.panelAA.actionVector.get(policy[i])+"\r\n");
