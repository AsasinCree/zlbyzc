...
public class ID3 {
	...
	private void  initPrintTree(){
		...
		c = new mgraphx(false, 22, 45, true);
		c.setSize(580, 800);
	...
	public void printDTree(TreeNode node) {
		/*
		 * // System.out.println(node.nodeName); System.out.println(
		 * "this is TreeNodeName "+node.nodeName+" childrens size "
		 * +node.childNodes.length); if (node.childNodes.length == 2) { if (flag
		 * == true) { this.mf.output.append(printStar(node.position)); } else {
		 * this.mf.output.append(printStar(15)); flag = true; node.position =
		 * node.parent.position + 13; } this.mf.output.append(node.nodeName +
		 * "\r\n"); TreeNode[] childs = node.childNodes;
		 * this.mf.output.append(printStar(node.position - 4));
		 * this.mf.output.append(childs[0].parentAttribute + "/");
		 * this.mf.output.append(printStar(5)); this.mf.output.append(
		 * "\\" + childs[1].parentAttribute + "\r\n"); childs[0].position =
		 * node.position - 10; for (int i = 0; i < childs.length; i++) { if
		 * (childs[i] != null) { // this.mf.output.append("\t"); //
		 * this.mf.output.append(childs[i].parentAttribute + // "\r\n"); //
		 * System.out.println(childs[i].parentAttribute); printDTree(childs[i]);
		 * } } } else if (node.childNodes.length == 1) {
		 * this.mf.output.append(printStar(node.position));
		 * this.mf.output.append(node.nodeName + "\r\n");
		 * this.mf.output.append(printStar(node.position - 3) +
		 * node.childNodes[0].parentAttribute + "\r\n");
		 * printDTree(node.childNodes[0]);
		 * 
		 * } else { if (flag == true) {
		 * this.mf.output.append(printStar(node.position));
		 * 
		 * // System.out.print(node.nodeName);
		 * this.mf.output.append(node.nodeName); flag = false; } else {
		 * this.mf.output.append(printStar(14)); //
		 * System.out.println(node.nodeName);
		 * this.mf.output.append(node.nodeName + "\r\n"); flag = true; }
		 * 
		 * }
		...
		if (node.parent == null) {
			...
			System.out.println(node.nodeName);
			Object n1=c.addNode(node.nodeName, 500, 40);
			
			
			String value = 500 + "_40";
		...
		if (node.childNodes.length > 0) {
			int jiange = 700 / node.childNodes.length;
