...
public class PanelA2search extends JPanel {
	String date;
	int focusedRowIndex;
	MainFrame mf = null;
	...
	JLabel searchResult = new JLabel("决策树（2）查询结果");
...
//			JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	String[] tableTitle = {"主题名称","行动数量","状态数量","子行动个数","保存时间 "};
	...
	public PanelA2search(MainFrame mf) {
		this.mf = mf;
	...
	public void searchPerformed(ActionEvent e) {
		...
		if(e.getSource() == delMenItem) {
    		...
    		rs = stmt.executeQuery("select * from chance_table where id = '"+ id +"' ");
    		while(rs.next()) {
    			
    			String ss ="delete from chance_table where id = '"+id+"'";
    ...
    //删除决策内容
     	  ...
     	  try{
     	   ...
     	   defaultModel.removeRow(focusedRowIndex);//删除表格选中那行
		}
...
//		JLabel allSearchNum = new JLabel("一共多少记录");
		...
		try{
			...
			allSearchNum.setText("一共" + rowCount + "记录" + "|");
			allSearchNum.updateUI();
//			//formTxt.append("主题名称	方案名称	属性数量	属性值个数" + "\r\n");
//			while(rs.next()) { 
//            	String ss1 = rs.getString("topicName");
//            	String ss2 = rs.getString("actionNum");
//            	String ss3 = rs.getString("stateNum");
		...
		scoreTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				 if (e.getButton() == java.awt.event.MouseEvent.BUTTON1) {
					...
					date = defaultModel.getValueAt(row, 4).toString();
				...
				//	searchInput.setText(themeName);
					...
					try {
						...
						conn = DriverManager.getConnection(mf.BR.setting.getConnURL("juece_2"));
						//conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/juece_2?useUnicode=true&characterEncoding=utf8", "root","111");
						Statement stmt = conn.createStatement();  
...
//					frame.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
				 ...
				 if (e.getButton() == java.awt.event.MouseEvent.BUTTON3) {  
			           ...
			           date = defaultModel.getValueAt(focusedRowIndex, 4).toString();
	...
	public void searchFromTheme(String date) {
		...
		try{
            ...
            if(rs.next()) {             	
            	...
            	ss = Integer.parseInt(rs.getString("stateNum"));
            	rr = Integer.parseInt(rs.getString("actionNum"));
            	mm = Integer.parseInt(rs.getString("childActionNum"));
            	sTxt.append("------主题名称：" + sTopicName +  "\r\n");
            	sTxt.append("保存时间：" + date +  "t" +"\t");
            	sTxt.append("------行动数量：" + ss +  "\t");
            	sTxt.append("------状态数量：" + rr +  "\t");
            	sTxt.append("------子行动数量：" + mm +  "\t\n");
            	sTxt.append("\r\n");
            	searchInput.setText(sTopicName);
            }
            sTxt.append( "\t");
            sTxt.append( "\t");            
            rs = stmt.executeQuery("select * from chance_table where id = '"+idd+"' "); 
            while(rs.next()) { 
            	String sss = rs.getString("state");
            	sTxt.append(sss + "\t");
            }
            sTxt.append( "\t\n");
            sTxt.append("概率" + "\t"+ "\t");            
            rs = stmt.executeQuery("select * from chance_table where id = '"+idd+"' "); 
            while(rs.next()) { 
            	String mmm = rs.getString("chance");
            	sTxt.append(mmm + "\t");
            }
            sTxt.append("\n");   
            String[]a = new String[rr*mm*ss] ;
       	 	String[]b = new String[rr*mm*ss] ; 
       	 	String[]c = new String[rr*mm*ss] ;        	   
        	rs = stmt.executeQuery("select * from benefit_table where id = '"+idd+"'");		
        		 int i=0;
        	while(rs.next()) {      			
        			String	qqq = rs.getString("action");
        			a[i]=qqq;
        			i++; 
        			System.out.print(i);
        		 }
        		...
        		while(rs.next()) {  
        			...
        			String	sss = rs.getString("childAction");
        			b[i]=sss;
        			i++;
        			}
        		 rs = stmt.executeQuery("select * from benefit_table where id = '"+idd+"'");
        		 ...
        		 while(rs.next()) {  
         			...
         			String	ddd = rs.getString("value");
         			c[i]=ddd;
         			i++;
         			}
        		for (int s=0;s<rr*mm;s++)
        		{	sTxt.append(a[s*ss] + "\t");
        			sTxt.append(b[s*ss] + "\t");
        			System.out.print(s);
        			for (int j = 0; j < ss; j++) {
        				sTxt.append(c[s*ss+j] + "\t");
					}
        			sTxt.append("\t\n");
        		}			
