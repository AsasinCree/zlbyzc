...
public class riskmatrixQueryPanel extends JPanel  {
	...
	private void layoutComponent() {
		...
		//查询结果详情 panel
		panelQueyResult.add(new JScrollPane(tableQueyResult),BorderLayout.CENTER);
		...
		panelQueyResult.add(panelLayoutButton,BorderLayout.SOUTH);
	...
	private void displayOnePage(){		
		...
		resTableModel.setColumnNames( new String[] {"序号","项目名称","项目日期","参与人员"} ); //设置table title
...
//		tableQueyResult.getColumnModel().getColumn(2).setMaxWidth(200); //日期 宽度
		//tableQueyResult.getColumnModel().getColumn(3).setMaxWidth(500); //日期 宽度
	...
	public void initQuery() {
		set_queryIndex(0);
		queryCmd = String.format("SELECT * FROM %s ORDER BY `ProjectID` DESC LIMIT %d, %d", "riskproject", 0, itemNumPerPage);
		displayOnePage();
		buttonEnableControl();
...
//				}		
				if (textfieldKeyword.getText().length() != 0){
					queryCmdBuf.append(String.format(" and ProjectName LIKE '%%%s%%'", textfieldKeyword.getText()));
				} 
				...
				queryCmdBuf.append(String.format(" ORDER BY `ProjectID` DESC LIMIT %d,%d", 0,itemNumPerPage));
				queryCmd = queryCmdBuf.toString().replaceFirst("and", "where");  //tricky: select * from table and ... and ... -->select * from table where ... and ...
...
//				}		
		...
		tableQueyResult.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {// 仅当鼠标单击时响应
				...
				String projectIDtext = tableQueyResult.getValueAt(r, 0).toString();
	//			String RiskProbability1 = (String) model.getValueAt(i, 4);
				int projectID = Integer.parseInt(projectIDtext.trim());
				String queryCMD = String.format("SELECT * FROM riskinformation WHERE ProjectID="+ projectID +"");
 
				JDBC_wapper mysql_ = new JDBC_wapper("conn.ini");
				ResultSet res = mysql_.query(queryCMD);
				ResultSetTableModel model2 = new ResultSetTableModel(res);
				model2.setColumnNames(new String[] {"序号","项目名称","风险名称","风险影响","风险影响量化值","风险概率","风险等级","Borda数","Borda序值","风险管控措施"});
		...
		buttonDelete.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < rowIds.length; i++) {
					...
					String deleteCmd = String.format("DELETE FROM %s where `ProjectID`= %d","riskproject",PK_id);
