...
public class riskmatrixQueryPanel extends JPanel  {
	private static final long serialVersionUID = 1L;
	
	private String tablename = "riskproject";  //table 设置
	...
	private JTable tableQueyResult2;

	public riskmatrixQueryPanel() {
		initializeComponent();
		layoutComponent();
		setEvents();
		showriskmatrixTable(); //added by lixuan
	}
	
	//点击查询按钮之前显示showriskmatrixTable的所有内容 added by 李璇
	private void showriskmatrixTable(){
		JDBC_wapper mysql_ = new JDBC_wapper("conn.ini");

		StringBuffer queryCmdBuf = new StringBuffer(String.format("SELECT DISTINCT * FROM %s", "riskproject"));
		...
		ResultSetTableModel model = new ResultSetTableModel(res);
				...
				model.setColumnNames( new String[] {"项目ID","项目名称","项目日期","参与人员"} ); //设置table title
				tableQueyResult.setModel(model);
				
				//if(tableQueyResult.getRowCount()==0){
					//tableQueyResult.setText("没有查询到相关记录");
				//}else {
	...
	private void layoutComponent() {
		...
		//查询结果详情 panel
		panelQueyDetail.add(new JScrollPane(tableQueyResult2));
		
		//JScrollPane scrollPane = new JScrollPane();
	//	panelQueyDetail.add(scrollPane);
		
//		tableQueyResult2 = new JTable();
//		tableQueyResult2.setModel(new DefaultTableModel(
//			new Object[][] {
//			},
//			new String[] {
//				"ID", "\u9879\u76EE\u540D\u79F0", "\u98CE\u9669\u540D\u79F0", "\u98CE\u9669\u5F71\u54CD", "\u98CE\u9669\u5F71\u54CD\u91CF\u5316\u503C", "\u98CE\u9669\u6982\u7387", "\u98CE\u9669\u7B49\u7EA7", "Borda\u6570", "Borda\u5E8F\u503C", "\u98CE\u9669\u7BA1\u63A7\u63AA\u65BD"
//			}
//		));
//		scrollPane.setColumnHeaderView(tableQueyResult2);
...
//				}
				...
				StringBuffer queryCmdBuf = new StringBuffer(String.format("select * from %s ", "riskproject"));			
				...
				ResultSet res = mysql_.query(queryCmd);
				ResultSetTableModel1 model = new ResultSetTableModel1(res);
...
//					
//				}
				String projectNameText = tableQueyResult.getValueAt(r, 1).toString();
				JDBC_wapper mysql_ = new JDBC_wapper("conn.ini");
	//			String queryCMD = String.format("select * from %s where PROJECTNAME ='%%%s%%'", "riskinformation", projectNameText);
				
	
				String queryCMD = String.format
						("SELECT * FROM riskinformation WHERE ProjectName='"+projectNameText+"'");
			//		String queryCMD = String.format("select * from %s ", "riskinformation");
				
				
				ResultSet res = mysql_.query(queryCMD);
				ResultSetTableModel1 model2 = new ResultSetTableModel1(res);
				model2.setColumnNames(new String[] {"ID","项目名称","风险名称","风险影响","风险影响量化值","风险概率","风险等级","Borda数","Borda序值","风险管控措施"} ); //设置table title
				tableQueyResult2.setModel(model2);		
...
//				textareaQueyDetail.setText(rowdes);
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					riskmatrixQueryPanel panelAhpQuery = new riskmatrixQueryPanel();
