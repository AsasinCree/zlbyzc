...
//\u4E34\u754C\u98CE\u9669\u503C
		tb_userdefineRiskLevel.setModel(new DefaultTableModel(
				...
				new String[] { "临界风险值（请输入整数）", "\u98CE\u9669\u7B49\u7EA7", "\u8BF4\u660E" }) {
					...
					// 使用用户自定义的风险等级
						...
						if (tb_userdefineRiskLevel.getValueAt(0, 0) == null) {
							...
							for (int i = 0; i < tb_userdefineRiskLevel.getRowCount(); i++) {
								float m;
								try{
									m = (float) tb_userdefineRiskLevel.getValueAt(i, 0);
								}catch(Exception e1) {
									JOptionPane.showMessageDialog(null,"请在自定义风险中输入浮点数或整数作为临界风险值");
						            e1.printStackTrace();
						            return ;
						        }
								if (d < m || d == m) {
		...
		button = new JButton("实 例");
		button.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				textField_name.setText("南海问题");
				textField_people.setText("张建 李明");
				tf_riskname.setText("触礁风险");
				...
				//String risk_name = tf_riskname.getText();
					...
					// null, null, null});
					int newRow = tb_lecd.getRowCount();
					model.setRowCount(tb_lecd.getRowCount() + 3);

					tb_lecd.setValueAt(newRow + 1, newRow, 1);
					tb_lecd.setValueAt("爆炸风险", newRow, 2);
					tb_lecd.setValueAt((float)0.5, newRow, 3);
					tb_lecd.setValueAt((float)6, newRow, 4);
					tb_lecd.setValueAt((float)100, newRow, 5);
					tb_lecd.setValueAt("300", newRow, 6);
					tb_lecd.setValueAt("4级", newRow, 7);
					
					
					tb_lecd.setValueAt(newRow + 2, newRow+1, 1);
					tb_lecd.setValueAt("沉船风险", newRow+1, 2);
					tb_lecd.setValueAt((float)1, newRow+1, 3);
					tb_lecd.setValueAt((float)2, newRow+1, 4);
					tb_lecd.setValueAt((float)40, newRow+1, 5);
					tb_lecd.setValueAt("80", newRow+1, 6);
					tb_lecd.setValueAt("3级", newRow+1, 7);
					
					tb_lecd.setValueAt(newRow + 3, newRow+2, 1);
					tb_lecd.setValueAt("碰撞风险", newRow+2, 2);
					tb_lecd.setValueAt((float)1, newRow+2, 3);
					tb_lecd.setValueAt((float)3, newRow+2, 4);
					tb_lecd.setValueAt((float)7, newRow+2, 5);
					tb_lecd.setValueAt("21", newRow+2, 6);
					tb_lecd.setValueAt("2级", newRow+2, 7);
...
//System.out.println(tb_lecd.getValueAt(i, 3).toString()+"****************");
					//float l = Float.parseFloat(tb_lecd.getValueAt(i, 3).toString()) ;
					...
					//float d1 = (String) ;
					float d = Float.parseFloat(((String)tb_lecd.getValueAt(i, 6)).trim());

					// ren: insert into 改为 insert
					String lecdsql = "insert lec_table (PROJECTNAME,PROJECTTIME,PEOPLE,RISKNAME,L,E,C,D,RISKLEVEL) values(" + "'" + projectName + "'," + "'" + projectTime + "',"
