...
public class LecQueryPanel extends JPanel  {
	...
	//private mx panelMgraphx; //右边ahp层次结构图
	...
	private int itemNumPerPage = 10;  //查询每页显示5条记录
	...
	private void layoutComponent() {
		...
		//sadd(panelMgraphx,BorderLayout.EAST);
	...
	private void displayOnePage(){		
		...
		resTableModel.setColumnNames( new String[] {"id","项目名称","项目时间","参与人员","风险名称","L","E","C","D","风险等级"} ); //设置table title
...
//		tableQueyResult.getColumnModel().getColumn(2).setMaxWidth(200); //日期 宽度
	...
	public void initQuery() {
		set_queryIndex(0);
		queryCmd = String.format("SELECT * FROM %s ORDER BY `id` DESC LIMIT %d, %d", "lec_table", 0, itemNumPerPage);
		displayOnePage();
		buttonEnableControl();
	...
	private void setEvents() {
		...
		buttonQuery.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				set_queryIndex(0);
				StringBuffer queryCmdBuf = new StringBuffer(String.format("SELECT * FROM %s", "lec_table"));				
//				if (key.length() == 0) {
		...
		buttonDelete.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < rowIds.length; i++) {
					...
					String deleteCmd = String.format("DELETE FROM %s where `id`= %d","lec_table",PK_id);
