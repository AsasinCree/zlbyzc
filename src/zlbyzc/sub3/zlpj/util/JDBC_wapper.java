...
public class JDBC_wapper {
	...
	public JDBC_wapper(String sIniPath){
		try {
	...
	public boolean insert(String insertCMD) {
		try {
			System.out.println("insertCMD: "+insertCMD);
...
//		}
	...
	public boolean delete(String deleteCMD) {
		try {
...
//	}
	...
	private  Connection makeConnection(String sIniPath)
		...
		FileInputStream in = new FileInputStream(MyPath.getPath()+File.separator+"db"+File.separator+sIniPath);
		props.load(in);
		in.close();
		...
		//取得数据库连接
		String connCMD = String.format("%s?user=%s&password=%s&useUnicode=true&characterEncoding=UTF8",url,username, password);
	...
	public static void testInsertAndQuery(){
		try {
			...
			String insertCMD = "insert ahp values('南海问题','20000302','一些结论等等')";
			mysql_.insert(insertCMD);
			
			String queryCMD = "select * from ahp";
	...
	public static void testDatetime(){
		try {
			...
			String insertCMD;	 
			insertCMD = String.format("insert ahp values('a1问题','%s','一些结论等等')", "2000-2-3");mysql_.insert(insertCMD);
			insertCMD = String.format("insert ahp values('a2问题','%s','一些结论等等')", "2000-2-4");mysql_.insert(insertCMD);
			insertCMD = String.format("insert ahp values('a3问题','%s','一些结论等等')", "2001-2-3");mysql_.insert(insertCMD);
			insertCMD = String.format("insert ahp values('ax问题','%s','一些结论等等')", "2010-12-3");mysql_.insert(insertCMD);
			insertCMD = String.format("insert ahp values('a5问题','%s','一些结论等等')", "2003-2-13");mysql_.insert(insertCMD);
			
			//System.out.println(insertCMD);
			//mysql_.insert(insertCMD);
			
			String queryCMD = "select * from ahp where aim like '%%%问题%%%' and argueTime>'20050202' and argueTime<'20150302'";
			ResultSet rs = mysql_.query(queryCMD);
