...
import zlbyzc.sub3.zlpj.anp.AnpJDBC_wapper;
...
public class DeaQueryFrm extends sub3inFrame  {
	...
	//点击查询按钮之前显示anptable的所有内容 
	private void showdeaTable(){
		AnpJDBC_wapper mysql_ = new AnpJDBC_wapper("conn.ini");

		StringBuffer queryCmdBuf = new StringBuffer(String.format("SELECT DISTINCT * FROM %s", "dea"));
		...
		model.setColumnNames( new String[] {"目标","日期","DEA模型","结论"} ); //设置table title
...
//				}
				...
				StringBuffer queryCmdBuf = new StringBuffer(String.format("SELECT DISTINCT * FROM %s", "dea"));
				if (key != "") {
					queryCmdBuf.append(String.format(" and deaaim LIKE '%%%s%%'", key));
				 }
