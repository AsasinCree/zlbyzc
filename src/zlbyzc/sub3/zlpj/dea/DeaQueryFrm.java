...
public class DeaQueryFrm extends sub3inFrame  {
...
//	private JScrollPane contentPaneC;
	...
	public DeaQueryFrm() {
		super("DEA数据包络分析历史查询",true,true,true,true);
...
//		add("Center",contentPaneC);
	...
	private void displayOnePage(){		
		...
		resTableModel.setColumnNames( new String[] {"id","目标","日期","DEA模型","结论"} ); //设置table title
...
//		tableQueyResult.getColumnModel().getColumn(2).setMaxWidth(500); //日期 宽度
	...
	public void initQuery() {
		set_queryIndex(0);
		queryCmd = String.format("SELECT * FROM %s ORDER BY `id` DESC LIMIT %d, %d", "dea", 0, itemNumPerPage);
		displayOnePage();
		buttonEnableControl();
	...
	buttonQuery.addActionListener(new ActionListener() {			
		...
		public void actionPerformed(ActionEvent e) {
			set_queryIndex(0);
			StringBuffer queryCmdBuf = new StringBuffer(String.format("SELECT * FROM %s", "dea"));				
//			if (key.length() == 0) {
//				JOptionPane.showMessageDialog(panelQureyCondition, "查询不能为空！");
//				return;
//			}		
			if (textfieldKeyword.getText().length() != 0){
				queryCmdBuf.append(String.format(" and deaaim LIKE '%%%s%%'", textfieldKeyword.getText()));
			} 
			...
			if (datePickerArgueStartDate.getModel().isSelected()) {
				...
				queryCmdBuf.append(String.format(" and deaargueTime>='%s'", startdate));
			}
			//结束日期
			if (datePickerArgueEndDate.getModel().isSelected()) {
				...
				queryCmdBuf.append(String.format(" and deaargueTime<='%s'", enddate));
			}
...
//			dbIO dbio=new dbIO("com.mysql.jdbc.Driver","jdbc:mysql://127.0.0.1:3306/db_zlpj","root","wipm");	    
			...
			for (int i = 0; i < rowIds.length; i++) {
				...
				String deleteCmd = String.format("DELETE FROM %s where `id`= %d","dea",PK_id);
