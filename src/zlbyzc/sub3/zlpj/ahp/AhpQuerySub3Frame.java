...
public class AhpQuerySub3Frame extends sub3inFrame  {
	...
	//查询条件组件
	private JPanel panelQuery; //左边查询面板 (宽度2/3)
	private mgraphx panelMgraphx; //右边ahp层次结构图 (宽度1/3)
	...
	public AhpQuerySub3Frame(BasicRibbon _br) {
	...
	private void layoutComponent() {
		...
		panelArgueEndDate.add(Box.createHorizontalStrut(10));
	...
	private void displayOnePage(){		
		...
		resTableModel.setColumnNames( new String[] {"id","目标","日期","AHP模型","图id"} ); //设置table title
		...
		tableQueyResult.getColumnModel().getColumn(3).setPreferredWidth((int) (BR_width*2/3* 0.53));		
	}
	...
	public void initQuery() {
		set_queryIndex(0);
		queryCmd = String.format("SELECT * FROM %s ORDER BY `id` DESC LIMIT %d, %d", "ahp", 0, itemNumPerPage);
		displayOnePage();
		buttonEnableControl();
	...
	private void setEvents() {
		...
		buttonQuery.addActionListener(new ActionListener() {			
			...
			public void actionPerformed(ActionEvent e) {
				set_queryIndex(0);
				StringBuffer queryCmdBuf = new StringBuffer(String.format("SELECT * FROM %s", "ahp"));				
//				if (key.length() == 0) {
...
//				}		
		...
		tableQueyResult.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {// 仅当鼠标单击时响应
				...
				textareaQueyDetail.setText(tableQueyResult.getValueAt(r, 3).toString()); //3即ahpModel在sqltable中的序号。
...
//				dbIO dbio=new dbIO("com.mysql.jdbc.Driver","jdbc:mysql://127.0.0.1:3306/db_zlpj","root","wipm");	    
				...
				for (int i = 0; i < rowIds.length; i++) {
					...
					Object gid = tableQueyResult.getModel().getValueAt(rowIds[i], 4);
					String deleteCmd = String.format("DELETE FROM %s where `id`= %d","ahp",PK_id);
