...
public class AnpDataPanel extends JPanel {
	 ...
	 * Create the frame.
	 */
	public AnpDataPanel(String aim, String[] factors,String[] plans,String time) {
...
//		paintPanel.add(matInfo);
		...
		factorMatInPanel.add(Box.createHorizontalStrut(500));
		...
		add(factorMatInPanel);
		add(plan2factorMatInPanel);
		add(factor2planMatInPanel);
		add(weightMatInPanel);
		add(panelLayButton);
				...
				int gid=-1;
				 try {
					 dbIO dbio=new dbIO("com.mysql.jdbc.Driver","jdbc:mysql://192.168.0.2:3336/db_zlpj",
				    			"root","wipm");	     	      			    	
					    gid = AnpInitFrm.conframe.c.gpanel.saveG2DB("",0,dbio);	
