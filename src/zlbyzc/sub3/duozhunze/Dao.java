...
public class Dao extends BaseDao {
	...
	public List querydescition(String keyword,int i) {
		...
		if(k){
			return this.queryList("from Descition as m  where  m.id like '%"+keyword+"%' or m.name like '%"+keyword+"%' or m.VarScheme like '%" + keyword + "%' or m.varLastConsistency >= '%"+n+"%' order by id desc",i);
			}else{
			return this.queryList("from Descition as m  where m.name like '%"+keyword+"%' or m.VarScheme like '%" + keyword + "%' order by id desc",i);
			}
	}
	...
	public int querydescition(String keyword) {
		...
		if(k){
		s=this.queryList("from Descition as m  where m.id like '%"+n+"%' or m.name like '%"+keyword+"%' or m.VarScheme like '%" + keyword + "%' or m.varLastConsistency >= '%"+n+"%' order by id desc");
		}else{
		s=this.queryList("from Descition as m  where m.name like '%"+keyword+"%' or m.VarScheme like '%" + keyword + "%' order by id desc");
		}
		for(Descition d:s){
			a++;
			}
	...
	public Object queryschemeByNumber2(int number) {
		return this.queryObject("from Scheme where id='" + number + "'");
	}
	//根据编号和方案名查找方案
	...
	//删除方案 根据id
	public void delefangan(int number,String name) {
		deleteOfBatch("delete from Scheme  where name='" + name + "' and descition_id= '" + number + "'");
	}
	//删除准则 根据id
		public void delezhunze(int number,String name) {
			deleteOfBatch("delete from Rule where descrition_id='" + number + "' and name = '" + name + "' ");
		}
	//根据编号查找专家
	public Object queryexpertByNumber(String number) {
		return this.queryObject("from Expert where Number='" + number + "'");
	}
	//根据名字查找决策
	...
	public Object queryDescitionbyName2(int Name){
		return  this.queryObject("from Descition where id='" + Name + "'");
	}
	//根据id查找决策
	...
	public List queryExpert(int i){return this.queryList("from Expert order by id desc",i);}
	//读取专家表个数+1
	...
	public Boolean add(Expert expert) {
		return this.saveObject(expert);
	}



	public void deletFromName(int valueAt) {
		// TODO Auto-generated method stub
		int id=valueAt;
		Descition des=(Descition)queryObject("from Descition where id='" + valueAt + "'");
		deleteOfBatch("delete from Rule where descrition_id="+id);
		deleteOfBatch("delete from Scheme where descition_id="+id);
		deleteOfBatch("delete from Descition where id="+id);
	}



	public void deleexpert(int i) {
		// TODO Auto-generated method stub
		deleteOfBatch("delete from Expert where Number ="+i);
		
	}



	public List<Expert> findExpertbyid(int id) {
		// TODO Auto-generated method stub
		List<Expert> f=new ArrayList<Expert>();
		List<Expert_Descition> s=this.queryList("from Expert_Descition where descition_id="+id);
		for(Expert_Descition D:s){
			f.add((Expert)this.queryObject("from Expert where id="+D.getExpert_id()));
		}
		return f;
	}
