...
public class GameTree extends sub3inFrame {
	 ...
	 * Create the panel.
	 */
	public GameTree(BasicRibbon _br) {
		...
		allcontent.add(lbBasicInfo, gbc_lbBasicInfo);
		...
		allcontent.add(lb_Basic_name, gbc_lb_Basic_name);
		...
		allcontent.add(tf_Basic_name, gbc_tf_Basic_name);
		...
		allcontent.add(btn_Basic_save, gbc_btn_Basic_save);
		...
		allcontent.add(lb_Gametree, gbc_lb_Gametree);
		...
		allcontent.add(btn_Gametree_update, gbc_btn_Gametree_update);
		...
		btn_Gametree_save.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					//删除已有策略和收益组
					String sql = "delete from tb_strategy where game_tree_id=" + game_tree_id;
				    stmt.executeUpdate(sql);
				    sql = "select * from tb_profit_set where game_tree_id=" + game_tree_id;
					ResultSet rs_profitd = stmt.executeQuery(sql);
					while(rs_profitd.next())
					{
						Statement stmt_p= connect.createStatement();
						int profit_set_id = rs_profitd.getInt("profit_set_id");
		...
		allcontent.add(btn_Gametree_save, gbc_btn_Gametree_save);
		...
		allcontent.add(btn_Gametree_compute, gbc_btn_Gametree_compute);
		...
		allcontent.add(btn_Gametree_result, gbc_btn_Gametree_result);
