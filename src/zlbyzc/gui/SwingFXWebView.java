...
//                  pdfViewer.openFile(new URL(newLoc))
        	 ...
        	 String[] downloadableExtensions = {".script", ".markdown",".pdf",".rst",".html"};
         	...
         	String pattern = "http(s?)://(.*)/nbconvert/(.*)/(.*)\\.ipynb\\?download=(.*)";
           // 创建 Pattern 对象
           Pattern r = Pattern.compile(pattern);
           // 现在创建 matcher 对象
           Matcher m = r.matcher(newLoc);
           if(m.find()){
              ...
              initialFileName=m.group(4);
            
                  downloadableExtension = "."+m.group(3);
                  System.out.println("Ext="+downloadableExtension);
            
           }
           else{
        	   pattern = "http(s?)://(.*)files/(.*)\\?download=(.*)";
               // 创建 Pattern 对象
               ...
               if(m.find()){
	               ...
	               System.out.println("Found value: " + m.group(3) );
	               System.out.println("Found value: " + m.group(4) );
	               downloadableExtension=".ipynb";
	               System.out.println("Ext="+downloadableExtension);
	               initialFileName=m.group(3);
               }
           }
            if (downloadableExtension != null) {
               ...
               (new FileChooser.ExtensionFilter("Downloadable File", downloadableExtension));
