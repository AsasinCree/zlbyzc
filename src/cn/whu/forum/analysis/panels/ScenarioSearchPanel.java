...
public class ScenarioSearchPanel extends JPanel implements ActionListener {
	...
	// 情景分析法查询结果的列表
	private List<ScenarioTask> commonList;
	// 当前操控的行号
	...
	private void initializeComponent() {
		...
		// width对应按钮占比宽度
		datePickerArgueTimeStartDate.setPreferredSize(StaticConfig.WIDTH_JDATEPICKER_BUTTON,
				StaticConfig.FONT_NEWTASK_TEXTFIELD.getSize() + 11);
		datePickerArgueTimeEndDate = new JDatePicker();
		datePickerArgueTimeEndDate.setFont(StaticConfig.FONT_NEWTASK_TEXTFIELD);
		// width对应按钮占比宽度
		datePickerArgueTimeEndDate.setPreferredSize(StaticConfig.WIDTH_JDATEPICKER_BUTTON,
				StaticConfig.FONT_NEWTASK_TEXTFIELD.getSize() + 11);
		...
		buttonDeleteScenarioTask.setFont(StaticConfig.FONT_SEARCHVIEW_BUTTON);
		buttonModifyScenarioTask = new JButton("修改");
		buttonModifyScenarioTask.setFont(StaticConfig.FONT_SEARCHVIEW_BUTTON);
		...
		scrollpaneTable.setPreferredSize(new Dimension(0, 550));
	...
	public void layoutComponent() {
		...
		panelLayEditButton.add(buttonDeleteScenarioTask);
		panelLayEditButton.add(Box.createHorizontalStrut(10));
		panelLayEditButton.add(buttonModifyScenarioTask);
	...
	public JTable initializeTable() {
		...
		dtm.setDataVector(new Object[][] { { "", "", "", "", "", -1 } },
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (table.getModel().getValueAt(selectedTableRow, 0) != "") {
					ScenarioTask scenarioTask = scenarioTaskDAO
							.getScenarioTaskByID((int) (table.getModel().getValueAt(selectedTableRow, 5)));
	...
	public void modifyTask() {
		...
		panelLayTaskDescriptionInModifyTask
				.setLayout(new BoxLayout(panelLayTaskDescriptionInModifyTask, BoxLayout.X_AXIS));
		...
		List<String> scenarioTaskLocationList = scenarioTaskDAO.getAllScenarioTasksLocation();
		for (String location : scenarioTaskLocationList)
		...
		if (table.getModel().getValueAt(selectedTableRow, 3) != null) {
			...
			cal.setTime(date);
			datePickerArgueTimeInModifyTask.setCustomTextFieldValue(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH),
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == buttonSearch) {
			...
			// 得到查询结果
			List<ScenarioTask> scenarioTaskListByLocation = scenarioTaskDAO
					.getScenarioTaskByLocation(textfieldTaskLocation.getText());
			List<ScenarioTask> scenarioTaskListByDate = scenarioTaskDAO.getScenarioTaskByDate(dateStartDate,
					dateEndDate);
			List<ScenarioTask> scenarioTaskListByName = scenarioTaskDAO
					.getScenarioTaskByName(textfieldTaskName.getText());
			...
			for (ScenarioTask list : scenarioTaskListByLocation) {
			...
			commonList = new ArrayList<ScenarioTask>();
			commonList.addAll(scenarioTaskListByDate);
				...
				if (i < commonList.size()) {
					...
					if (dtm.getValueAt(0, 0) == "") {
						...
						if (commonList.get(i).getArgueTime() != null) {
							SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
							String stringDate = sdf.format(commonList.get(i).getArgueTime());
							dtm.addRow(new Object[] { dtm.getRowCount() + 1, commonList.get(i).getTaskName(),
									commonList.get(i).getTaskDescription(), stringDate,
									commonList.get(i).getTaskLocation(), commonList.get(i).getTaskID() });
						} else
							dtm.addRow(new Object[] { dtm.getRowCount() + 1, commonList.get(i).getTaskName(),
									commonList.get(i).getTaskDescription(), "", commonList.get(i).getTaskLocation(),
									commonList.get(i).getTaskID() });
					}
				}
			}
			...
			calendar.set(datePickerArgueTimeInModifyTask.getModel().getYear(),
					datePickerArgueTimeInModifyTask.getModel().getMonth(),
					datePickerArgueTimeInModifyTask.getModel().getDay(), 0, 0, 0);
			calendar.set(Calendar.MILLISECOND, 0);

			ScenarioTask scenarioTask = new ScenarioTask();
			// 获得remove掉的列的data必须使用TableModel
			scenarioTask.setTaskID((int) (table.getModel().getValueAt(selectedTableRow, 5)));
			...
			dtm.setValueAt(scenarioTask.getTaskName(), selectedTableRow, 1);
			dtm.setValueAt(scenarioTask.getTaskDescription(), selectedTableRow, 2);
			...
			dtm.setValueAt(scenarioTask.getTaskLocation(), selectedTableRow, 4);
			dtm.setValueAt(scenarioTask.getTaskID(), selectedTableRow, 5);
